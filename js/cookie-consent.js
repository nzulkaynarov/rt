/**
 * RailTech — Cookie / Storage Consent Banner
 *
 * What this site actually stores:
 *  - localStorage['railtech_lang']          — language preference (essential)
 *  - localStorage['railtech_cookie_consent'] — this consent record (essential)
 *
 * "Accept all" also enables future analytics/tracking when added.
 * "Essential only" restricts to the two items above.
 *
 * Dispatches CustomEvent('cookieconsent', { detail: { level: 'all'|'essential' } })
 * so analytics scripts can conditionally activate.
 */
(function () {
    'use strict';
    ...
    // Already decided for this version → fire event and exit
    const stored = getStored();
    if (stored && stored.version === CONSENT_VERSION) {
        document.dispatchEvent(new CustomEvent('cookieconsent', { detail: { level: stored.level } }));
        return;
    }

    // ── i18n helpers (works before and after i18n.js) ───
    const COPY = {
        ru: {
        ...
        uz: {
            title:   'Cookie fayllari va mahalliy xotira',
    ...
    function getLang() {
        return (localStorage.getItem('railtech_lang') ||
    ...
    // ── Build banner ─────────────────────────────────
    function showBanner() {
        // Slight delay so the page paints first (better UX)
...
@media (max-width: 600px) {
    .cb-inner { padding: 14px 16px; gap: 12px; }
...
<div class="cb-inner">
    ...
    <div class="cb-actions">
        ...
        // If user switches language while banner is visible — rebuild it
        document.addEventListener('langchange', function rebuild() {
    ...
    // Wait for DOMContentLoaded + a short delay so the page renders first
