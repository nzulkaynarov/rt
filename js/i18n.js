/**
 * RailTech Landing — i18n engine
 * Works in file://, GitHub Pages, and nginx contexts.
 * Translations are bundled inline (as fallback) AND loaded from locales/*.json
 * when served via HTTP for easy editing without touching JS.
 */
(function () {
    'use strict';
    ...
    /* ─── Inline fallback translations ──────────────────────────────────
       These are used when fetch is unavailable (file:// protocol) or the
       locales/ folder can't be reached. Update locales/*.json for content
       changes — JS will prefer the fetched version when available.
    ─────────────────────────────────────────────────────────────────── */
    const INLINE = {
        ru: {
            nav_about:"О компании", nav_services:"Услуги", nav_platform:"Платформа",
        ...
        en: {
            nav_about:"About", nav_services:"Services", nav_platform:"Platform",
        ...
        uz: {
            nav_about:"Kompaniya", nav_services:"Xizmatlar", nav_platform:"Platforma",
    ...
    async function init() {
        ...
        document.querySelectorAll('.js-lang-switcher').forEach(el => {
            el.value = lang;
